...
def get_chat():
    ...
    try:
        ...
        if chat_type == 'private':
            chat_name = result.get('username', result.get('first_name', 'Unknown'))
            log.logger.info(f"Telegram getChat successful. Chat User: [{chat_name}], type: {chat_type}")
        elif chat_type == 'channel':
            chat_name = result.get('title', 'Unknown')
            log.logger.info(f"Telegram getChat successful. Chat title: [{chat_name}], type: {chat_type}")
        else:
            chat_name = result.get('title', 'Unknown')
